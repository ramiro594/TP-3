...
export function leerSuperheroes(ruta) {
  try {
    ...
    if (!Array.isArray(superheroesArray)) {
      throw new Error(
        "El archivo no contiene una lista válida de superhéroes."
      );
    }
...
export function agregarSuperheroe(rutaOriginal, rutaNuevos) {
  try {
    ...
    if (
      !Array.isArray(superheroesOriginales) ||
      !Array.isArray(superheroesNuevos)
    ) {
      throw new Error(
        "Uno de los archivos no contiene una lista válida de superhéroes."
      );
    }

    // Combinar las listas de superhéroes originales y nuevos, eliminando duplicados por id
    const listaCompleta = [
      ...superheroesOriginales,
      ...superheroesNuevos,
    ].filter(
